...
export function send(message, res) {
  transporter.sendMail(message, (error, info) => {
    if (error) {
      logy("Error occurred trying to send");
      logy(error.message);
      return process.exit(1);
    }

    logy("Message sent successfully!--------------------------------");
    logy(message.to);
...
export function sendTemplate(message, res) {
  let template = readFileSync(__dirname.replace("controllers/lib", "views") + "/mails/base.html", { encoding: "utf-8" });

  let templateText = $(template);
  templateText.find("#uLogo").attr("src", "http://" + _host + "/img/logom2.png");
  ...
  // transporter.sendMail(message, (error, info) => {
  // 	if (error) {
  // 		logy('Error occurred trying to send template');
  // 		logy(error.message);
  // 		return false;
  // 	}
